...
const breadProducts = products.reduce((acc, curr) => {
  if (curr.name.includes("Bread")) {
    acc.push(curr.name);
  }
  return acc;
}, []);
...
const avgSalary = totalSalary.total / employees.length

const employeesWithNewProperty = employees.map((employee) => ({...employee, aboveAverage: employee.salary > avgSalary }))
console.log(employeesWithNewProperty)
console.log("\n---- ---- ----");
...
const mostActive = community.reduce((pre, curr) => {
  const preTotalHrs = pre.hours.reduce((acc, curr) => acc + curr , 0)
  const currTotalHours = curr.hours.reduce((acc, curr) => acc + curr, 0)
  return preTotalHrs > currTotalHours ? pre : curr
})
...
const taggedCommunity = communityData.map(member => {
  const totalHours = member.hours.reduce((acc, curr) => acc + curr , 0);
  if(member.role === "student" && totalHours > 20){
    return {...member, tag: "regular"}
  }
  return member
})
console.log(taggedCommunity)